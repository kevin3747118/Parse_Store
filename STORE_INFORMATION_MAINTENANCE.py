            ...
            if html_result1.find_all('span', class_="_Xbe"):
                ...
                if html_result1.find_all('td', class_="_X0c"):
                    for week_tag in html_result1.find_all('td', class_="_X0c"):
                        ### verfiry week info with festival or not
                ...
                ###convert db_store_infos and week_infos_dict to tuple###
                sql_stat = ("""UPDATE [dbo].[STORE_INFORMATION_MAINTENANCE]
...
class get_store_data(scrapy_need):
    ...
    def __get_store_area(self):
        ...
        sql_stat = ("""select distinct(地區名稱) from [dbo].[STORE_INFORMATION_MAINTENANCE]
    ...
    def get_single_store(self):
        ...
        for i in range(len(__single_area)):
            ...
            sql_stat = ("""SELECT [PSD_NUM], [特約商店名稱], [地址], [縣市名稱], [地區名稱]
                           from [dbo].[STORE_INFORMATION_MAINTENANCE]
                           where [特約商店名稱] in (select [特約商店名稱] FROM [dbo].[STORE_INFORMATION_MAINTENANCE]
                           group by [特約商店名稱] having count(*) <= 1 )
                           and [地區名稱] = %s and [分店名稱] = '' and [google電話] is null""")
    ...
    def get_multiple_store(self):
        ...
        for i in range(len(__multiple_area)):
            ...
            sql_stat = ("""SELECT [PSD_NUM], [特約商店名稱], [分店名稱], [地址], [縣市名稱], [地區名稱]
                           from [dbo].[STORE_INFORMATION_MAINTENANCE]
                           where [地區名稱] = %s and [google電話] is null """)
